...
namespace Tarbora {
    ...
    void SceneNode::Update(Scene *scene, float deltaTime)
    {
        ...
        if (m_Position != m_TargetPosition)
        {
            ...
            if (InterpolatePosition(m_PositionCounter/m_PositionTime))
            {
                m_PositionCounter = 0.0f;
            }
        }
    ...
    {
        Move(glm::vec3(position.x - m_Position.x, position.y - m_Position.y, position.z - m_Position.z));
    }
    ...
    bool SceneNode::InterpolatePosition(float fraction)
    {
        ...
        glm::vec3 deltaPos = fraction*(m_TargetPosition - m_OldPosition);
        m_LocalMatrix = glm::translate(m_LocalMatrix, deltaPos);
        m_Position = m_OldPosition + deltaPos;
        return false;
    }
