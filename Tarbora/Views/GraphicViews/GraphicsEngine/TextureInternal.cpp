...
namespace Tarbora {
  ...
  TextureInternal::TextureInternal(
    int width, int height, unsigned int internal_format,
    unsigned int type, unsigned int format, void *data
  )
  {
    width_ = width;
    height_ = height;
  ...
  void TextureInternal::configure(
  ...
  )
  {
    ...
    glBindTexture(GL_TEXTURE_2D, id_);
    glTexImage2D(
      GL_TEXTURE_2D, 0, internal_format, width_, height_, 0, format, type, data
    );
