...
namespace Tarbora {
    ...
    void Renderer::OcclusionPass()
    {
        ...
        m_OcclusionShader->Set("projection", m_Projection);
        m_OcclusionShader->Set("view", m_View);
        m_gPosition->Bind(0);
        m_gNormal->Bind(1);
        ...
        m_OcclusionBlurShader->Set("projection", m_Projection);
        m_OcclusionBlurShader->Set("view", m_View);
        m_ssaoColor->Bind();
        glBindVertexArray(m_QuadMesh->GetId());
        glDrawArrays(GL_TRIANGLES, 0, m_QuadMesh->GetVertices());
    }
    ...
    void Renderer::LightingPass()
    {
        ...
        m_LightingShader->Set("projection", m_Projection);
        m_LightingShader->Set("view", m_View);
        m_gPosition->Bind(0);
        m_gNormal->Bind(1);
        m_gColorSpec->Bind(2);
    ...
    void Renderer::ScenePass()
    {
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        m_SceneShader->Use();
        m_SceneShader->Set("projection", m_Projection);
        m_SceneShader->Set("view", m_View);
    ...
    void Renderer::Postprocess()
    {
        ...
        m_PostprocessShader->Set("projection", m_Projection);
        m_PostprocessShader->Set("view", m_View);
        m_SceneColor->Bind();
        glBindVertexArray(m_QuadMesh->GetId());
        glDrawArrays(GL_TRIANGLES, 0, m_QuadMesh->GetVertices());
    }
