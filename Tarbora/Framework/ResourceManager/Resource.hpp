...
bool operator==(std::nullptr_t) const {return (name_ == "") || ResourceManager::getResource(name_) == nullptr;
    }
    ...
    std::shared_ptr<T> operator->() const
    {
      ...
      std::shared_ptr<T> resource = std::static_pointer_cast<T>(
        ResourceManager::getResource(name_, &just_loaded)
      );
      if (resource == nullptr && fallback_name_ != "")
      {
        resource = std::static_pointer_cast<T>(
          ResourceManager::getResource(fallback_name_, &just_loaded)
        );
      }
